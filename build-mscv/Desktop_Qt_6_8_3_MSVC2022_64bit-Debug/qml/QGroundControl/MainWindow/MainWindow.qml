...
ApplicationWindow {
    id: mainWindow
    visible: false // Başlanğıcda pəncərəni gizlədirik ki, splash screen görünsün
    width: Screen.width
    ...
    // Qalan ApplicationWindow komponentləri və funksionallığı
    // Bu hissələr əvvəlki kodunuzda olduğu kimi qalır
    ...
    function hideToolDrawer() {
            if (toolDrawer.visible) { // Əgər toolDrawer görünürsə
                toolDrawer.visible = false // Onu gizlədirik
                toolDrawerLoader.source = "" // Və yüklənmiş məzmunu yaddaşdan silirik
            }
        }
    ...
    PlanView {
        id:              planView
           // anchors.fill: parent    <-- bunu silirik
           width:           parent.width - 250  // ya da istədiyin ölçü
    ...
    TopStatusPanel {
          ...
          width: parent.width - leftPanel.width // BURADA DƏYİŞİKLİK ETMƏK LAZIMDIR
          anchors.top: parent.top
          anchors.left: leftPanel.right         // BURADA DƏYİŞİKLİK ETMƏK LAZIMDIR
    ...
    Component {
        ...
        Window {
            ...
            height:      ScreenTools.defaultFontPixelHeight * 70
            // ... (qalan Window xüsusiyyətləri)
        }
    }
}
